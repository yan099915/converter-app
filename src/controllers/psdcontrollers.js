...
const application = require("../services/psdservices");
...
module.exports = {
  maincontroller: async (req, res) => {
    ...
    for (i = 0; i < children.length; i++) {
      ...
      for (j = 0; j < toplayer.length; j++) {
        if (toplayer[j].isGroup()) {
          ...
          if (!object.text) {
            ...
            file.objects.push(textObject);
          }
        }
      }
      // grouping
      file.objects.push(grouping);
      ...
      const saveJson = await application.saveDb(data);
    ...
    res.status(201).send({
      message: "file uploaded now you can search data with your zip filename",
    });
  },
